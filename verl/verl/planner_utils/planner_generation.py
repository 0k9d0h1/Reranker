...
class PlannerGenerationManager:
    def __init__(
        self,
        tokenizer,
        actor_rollout_wg,
    ...
    def _extract_answer_from_thought_string(self, text: str, prev_search):
        if not prev_search:
            return None
    ...
    def run_llm_loop(
        ...
        rollings = gen_batch
        # self.every_refine_mask = torch.zeros(gen_batch.batch["input_ids"].shape[0], dtype=torch.bool)
        is_search = None
        ...
        for step in range(self.config.max_turns):
            ...
            golden_answers = np.array(golden_answers) if golden_answers is not None else None
        ...
        # meta_info["refine_ratio"] = refine_cnt / len(active_mask) if len(active_mask) > 0 else 0
...
If I want to give the final answer, I should put the answer between <answer> and </answer>. Let me try again.\n"
    ...
    def batch_search(self, queries: List[str] = None) -> str:
        ...
        """
        data_batched = self._batch_search(queries)
        rerank_text_batched = []
        for data in data_batched:
            rerank_text = ""
            # rerank_text += "<information>\n"
            for idx, (document, reasoning) in enumerate(zip(data["documents"][:self.config.topk], data["reasonings"][:self.config.topk], strict=False)):
                content = document["text"]
                if self.config.return_full_documents:
                    title = content.split("\n")[0]
                    text = "\n".join(content.split("\n")[1:])
                    rerank_text += (
                ...
                # rerank_text += "</information>\n"
            rerank_text_batched.append(rerank_text)
        return rerank_text_batched
