...
from utils import (
    ...
    get_qrels_dl22,
    get_topics_dl22,
    get_topics_qrels_excluded_ids_for_bright,
    get_topics_qrels_for_r2med,
)
from dataclasses import dataclass, field
from trec_eval import Eval
from index_and_topics import (
    THE_TOPICS,
    THE_INDEX,
    THE_QRELS,
    BRIGHT_DATASETS,
...
class Arguments:
    ...
    output_format: Optional[str] = field(
        default="trec", metadata={"help": "Output format."}
    )
    retrieval_method: RetrievalMethod = field(
        default=RetrievalMethod.BM25, metadata={"help": "Method of retrieval."}
    )
    retrieval_results_name: str = field(
        default=None, metadata={"help": "the filename of custom retrieval results"}
    )
    retrieval_num: int = field(default=100, metadata={"help": "retrieval number"})
    rerank_topk: int = field(
        default=None, metadata={"help": "only need to rerank top-k candidates"}
    )
    threads: int = field(default=30, metadata={"help": "Number of threads."})
    batchsize_retrieval: int = field(
        default=32, metadata={"help": "batchsize for dense retrieval"}
    )
    reasoning_maxlen: int = field(
        default=1500, metadata={"help": "the max length of reasoning chain to generate"}
    )
    remove_query: Optional[bool] = field(
        default=True, metadata={"help": "Remove query from output."}
    )
    save_first_stage_run: Optional[bool] = field(
        default=True, metadata={"help": "Save first-stage run."}
    )
    remove_duplicates: Optional[bool] = field(
        default=False, metadata={"help": "Remove duplicates from output."}
    )
    shuffle_candidates: bool = field(
        default=False,
        metadata={"help": "Whether to shuffle the candidates before reranking."},
    )
    api_resource: str = field(default="baidu", metadata={"help": ""})
    use_gpt4cot_retrieval: bool = field(
        default=True,
        metadata={
            "help": "Whether to use the retrieval results of gpt4 cot for BRIGHT datasets (otherwise use the original query)."
        },
    )

    # llm arguments
    model_path: str = field(
        default=f"{WORKSPACE_DIR}/llm/rank_vicuna_7b_v1",
        metadata={
    ...
    max_lora_rank: int = field(
        default=32, metadata={"help": "one parameter of vllm initialization"}
    )
    use_azure_openai: bool = field(
        default=False,
        metadata={
            "help": "If True, use Azure OpenAI. Requires env var to be set: `AZURE_OPENAI_API_VERSION`, `AZURE_OPENAI_API_BASE`"
        },
    )
    context_size: int = field(
        default=4096, metadata={"help": "context size used for model."}
    )
    num_gpus: int = field(default=1, metadata={"help": "the number of GPUs to use"})
    cache_dir: Optional[str] = field(
        default="../../cache", metadata={"help": "Path to cache directory."}
    )
    ...
    num_passes: int = field(
        default=1, metadata={"help": "Number of passes to run the model."}
    )
    window_size: int = field(
        default=20, metadata={"help": "Window size for the sliding window approach."}
    )
    step_size: int = field(
        default=10, metadata={"help": "Step size for the sliding window approach."}
    )
    vllm_batched: bool = field(
        default=False, metadata={"help": "Whether to run the model in batches."}
    )
...
if __name__ == "__main__":
    ...
    if len(set(args.datasets) & set(BRIGHT_DATASETS)) >= 1:
        all_bright_examples = load_dataset(
            "xlangai/bright", "examples", cache_dir=f"{WORKSPACE_DIR}/data/bright"
        )
    for dataset in args.datasets:
        if dataset in BRIGHT_DATASETS:  # BRIGHT
            ...
            topics = (
                {
                    str(qid): content["title"]
                    for qid, content in get_topics(topics_path).items()
                }
                if dataset != "dl22"
                else get_topics_dl22()
            )  # dl22 is not supported by pyserini 0.20.0
            qrels = (
                {
                    str(qid): {str(docid): int(score) for docid, score in docs.items()}
                    for qid, docs in get_qrels(qrels_path).items()
                }
                if dataset != "dl22"
                else get_qrels_dl22()
            )
        ...
        if os.path.exists(first_state_run_path):
            ...
            with open(first_state_run_path, "r") as f:
                ...
                for line in f:
                    ...
                    ):  # for stackoverflow dataset, there are several docids containing white space (e.g., pytorch_torch_tensor_functions/Memory Management_2_0.txt), we just filter these docids
                        continue
                    qid, _, docid, _, score, _ = line.strip().split()
                    if qid != current_qid:
                        if current_qid is not None:
                            current_query = Query(
                                qid=current_qid, text=topics[current_qid]
                            )
                            results.append(
                                Request(
                                    query=current_query,
                                    candidates=current_ranking[: args.retrieval_num],
                                )
                            )
                        current_ranking = []
                        current_qid = qid
                    current_ranking.append(
                        Candidate(
                            ...
                            if dataset in BRIGHT_DATASETS + r2med_DATASETS
                            else json.loads(searcher.doc(docid).raw()),
                        )
                    )
            ...
            max_excluded_ids_num = max(
                [len(excluded_ids[qid]) for qid in batch_topic_ids]
            )
            _results = searcher.batch_search(
                batch_topics,
                batch_topic_ids,
                k=args.retrieval_num + max_excluded_ids_num,
                threads=args.threads,
            )  # we need to filter the excluded_ids while keeping the retrieved number as retrieval_num
            results = []
            for topic_id in batch_topic_ids:
                candidates = [
                    ...
                    if result.docid not in excluded_ids[topic_id]
                ][: args.retrieval_num]
                results.append(
                    Request(
                        query=Query(qid=topic_id, text=topics[topic_id]),
            ...
            if args.save_first_stage_run:
                output_writer = get_output_writer(
                    first_state_run_path,
                    OutputFormat(args.output_format),
                    "w",
                    max_hits=args.retrieval_num,
                    tag=args.retrieval_method,
                    topics=topics,
                )
    ...
    if args.model_path in [
        ...
        agent = SafeOpenai(
            ...
            keys=get_openai_api_key(
                resource=args.api_resource, model_name=args.model_path
            ),
        )
    else:
    ...
    for dataset in args.datasets:
        ...
        for pass_ct in range(args.num_passes):
            ...
            results_grouped = [
                results_with_100_passages[i : i + args.batch_size]
                for i in range(0, len(results_with_100_passages), args.batch_size)
            ] + results_less_100_passages
            reranked_results = []
            rerank_details = []
            for batch in results_grouped:
                reranked_batch, time_cost, rerank_details_batch = reranker.rerank_batch(
                    ...
                    rank_end=args.retrieval_num
                    if args.rerank_topk is None
                    else args.rerank_topk,
