...
def get_topics_qrels_excluded_ids_for_bright(
    ...
    for example in examples:
        ...
        reldocids = (
            example["gold_ids"] if is_long_doc == False else example["gold_ids_long"]
        )
...
def convert_doc_to_prompt_content(
    tokenizer, doc: Dict[str, Any], max_length: int, truncate_by_word=True
) -> str:
    if "text" in doc:
        content = doc["text"]
    elif "segment" in doc:
    ...
    if truncate_by_word:
        ...
        content = tokenizer.convert_tokens_to_string(
            tokenizer.tokenize(content)[:max_length]
        )
...
def get_output_writer(
    file_path: str, output_format: OutputFormat, *args, **kwargs
) -> OutputWriter:
    mapping = {
