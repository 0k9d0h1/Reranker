...
try:
    ...
    print(
        "Seems vllm is not installed, RankR1SetwiseLlmRanker only supports vllm inference so far."
    )
...
class RankR1SetwiseLlmRanker(SetwiseLlmRanker):
    ...
    def __init__(
        ...
        if scoring != "generation":
            raise NotImplementedError(
                f"Scoring method {scoring} is not supported for RankR1SetwiseLlmRanker. RankR1SetwiseLlmRanker only supports 'generation' scoring."
            )
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            tokenizer_name_or_path, cache_dir=cache_dir
        )
        self.llm = LLM(
    ...
    def compare(self, query: str, docs: List):
        ...
        batch_data = []
        for _ in range(self.num_permutation):
            batch_data.append([random.sample(id_passage, len(id_passage)), labels])
        ...
        for batch in batch_data:
            ...
            batch_ref.append((ref, characters))
            passages = "\n".join(
                [f"{characters[i]} {passages[i]}" for i in range(len(passages))]
            )
            system_message = self.prompt["prompt_system"]
            user_message = self.prompt["prompt_user"].format(query=query, docs=passages)
            input_text.append(
                [
        ...
        outputs = self.llm.chat(
            ...
            use_tqdm=False,
            lora_request=LoRARequest("R1adapter", 1, self.lora_path)
